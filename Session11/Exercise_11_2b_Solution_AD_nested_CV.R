...
####   Logistic regularized regression - Practical 2b  ####
...
######################################################################################################################
# The outcome needs to be the last variable in the data set ###
###############################################################
...
x<-predictors
y<-diagnosis
# Merge x and y in a new dataframe object called data
data<-as.data.frame(cbind(x,y))
# The outcome needs to be defined as a factor
# Our outcome is already defined as a factor but this is how you can do it:
data$y<-as.factor(y) 
data$y
# Comment: levels 1 and 2 (or other numbers) aren't valid names in R and you need to label your two levels.
levels(data$y) <- c("AD", "Control")

# Important:
# Change the level so that control group is lower level, important to get correct sensitivity, specificity, PPV and NPV 
data$y<-relevel(data$y, ref = "Control")
...
#### FUNCTIONS NEEDED: ###############################################################################
## This a user-written functions which need to be run before you start the analyses (bottom of script)

#### Function 1: testPerformance()  ################################# 
...
##################################################################################################################################################
####### End of functions #########################################################################################################################
####### Run the analyses form here:

# To do: 
# Define the number of cores: cl=makeCluster(N)
# Set up the number of outer loops, outer repeats, inner loops , inner repeats
# Define the data set Data= XXX
# Check that the grid system is defined
# Set up a seed number to replicate the results: seed = XXX
...
[1] "The nested CV performance measures are: "
> print(NestedCV_results$testPerformance)
#Av.best Av.tol.1SE
#Av.AUC                0.91213    0.90735
#Av.sensBestThresh     0.74898    0.75979
#Av.specBestThresh     0.91069    0.91687
#Av.PPVBestThresh      0.76521    0.77676
#Av.NPVBestThresh      0.90335    0.90802
#Av.AccuracyBestThresh 0.86485    0.87217
#Av.KappaBestThresh    0.65827    0.67322
#Av.sens50Thresh       0.65261    0.60266
#Av.spec50Thresh       0.95753    0.97636
#Av.PPV50Thresh        0.84955    0.90810
#Av.NPV50Thresh        0.87538    0.86290
#Av.Accuracy50Thresh   0.86923    0.87217#
#Av.Kappa50Thresh      0.64664    0.63499
#Av.Beta               1.38240    1.98406
#Av.Alpha              0.03454    0.02105

# Summary of results:
# The AUC of the minimum lambda model is 91.2% (1Se=90.7). Sensitivity and Specificity at the 50%
# cut-off are 65% and 96%, resp., and the overall accuracy is 87%.The prediction accuracy measures are lower than 
# those of the apparent validity but still good. It should be noted that the classification
# task to separate people with AD and control is not very difficult and we will not see very often
# such good models.
# The calibration slope beta is 1.4 and calibration in the large = 0.03. 
